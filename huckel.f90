...
WRITE(*,*) 'This program compute the MO energy and coefficients, &
            using Hückel approximation'
WRITE(*,*) 'The data file would be written as the templates given in &
            the zip file with these code'
...
!-----------------------------------------------------------------------
!---- Call the subroutine.
!---- H is the initial matrix, X will have the eigenvectors, e is the
!---- tolerance to stop adn atoms is the dimension of the matrix

CALL jacobi(H,X,e,atoms)

!-----------------------------------------------------------------------
!---- Sort the resutls, the lowest energy is the last row
...
DO i = 1, atoms
  ...
  DO j = 1, atoms
    Info(i,j+1) = X(j,i)        !column vectors to row vectors
  ENDDO
ENDDO
...
DO i = 1, atoms
  DO j = i+1, atoms
    IF (Info(i,1) .LT. Info(j,1)) THEN          !larger value at the top
      V(:) = Info(i,:)             
...
!-----------------------------------------------------------------------
!---- Let's have the eigenvalues as a sum of alpha & beta values
...
OPEN(11,FILE=outputfile)
  ...
  DO i = 1, atoms
    IF (Gama(i) .LT. 0.d0) THEN !to print + & - without doubles + or -
      WRITE(11,FMT=f) 'E_', atoms+1-i, '= alpha -', -Gama(i), &
                                                    'beta = ', Info(i,1)
    ELSE
      WRITE(11,FMT=f) 'E_', atoms+1-i, '= alpha +',  Gama(i), &
                                                    'beta = ', Info(i,1)
  ...
  WRITE(11,FMT='(A11,*(A4,I2,A2))') '*******    ', &
                                              ('phi_',j,'  ', j=1,atoms)
  DO i = 1, atoms
    WRITE(11,FMT='(A4,I2,A3,*(F8.4))') 'Psi_', atoms+1-i, '= ', &
                                                (Info(i,j+1), j=1,atoms)
  ENDDO

  WRITE(11,*) ' '
  WRITE(11,*) '****************************************'
  WRITE(11,FMT='(A29,F9.4,A3)') ' The energy of pi system is: ', &
                                                         energy_s, ' eV'
  WRITE(11,*) '****************************************'
  WRITE(11,*) ' '
...
WRITE(*,*) 'The results are in: ', outputfile
...
IMPLICIT NONE
INTEGER                :: i, j, k, n
REAL*8                 :: e, b2, bar, ttheta, coeff, c, s, cs, sc
REAL*8                 :: t, aux
...
b2 = 0.d0                          !sum for no-diagonal elements squared
DO i = 1, n
  DO j = 1, n
    IF ( i .NE. j ) THEN
      b2 = b2 + H(i,j)*H(i,j)
...
!-----------------------------------------------------------------------
!--  e says if it's ok, i.e., when no-diagonal is like 0: diagonalizaded
IF (b2 .LE. e) RETURN

bar = 0.5*b2/FLOAT(n*n)                     !average for off-diagonal /2

DO WHILE (b2 .GT. e)                        !until converge
  DO i = 1, n-1
    DO j = i+1, n
      IF (H(j,i)**2 .LE. bar) CYCLE         !no with small elements
      b2  = b2 - 2.d0*H(j,i)*H(j,i)
      bar = 0.5*b2/FLOAT(n*n)
      !Compute with the given matrix
      ttheta = (H(j,j)-H(i,i))/(2.d0*H(j,i))
      coeff  = 0.5*ttheta/DSQRT(1.d0 + ttheta*ttheta)
      s = DSQRT(MAX(0.5 + coeff, 0.d0))              !sin
      c = DSQRT(MAX(0.5 - coeff, 0.d0))              !cos
